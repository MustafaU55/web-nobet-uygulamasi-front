...
const LogoutButton = () => {
    ...
    const { showError } = useNotification(); // Bildirim fonksiyonları

    const logout = async () => {
        setLoading(true);
        try {
            // API ile çıkış işlemi yapılabilir. Örneğin:
            await api.post('/api/logout', {
                withCredentials: true,
            });

            // Çıkış sonrası login sayfasına yönlendir
            window.location.href = '/login'; // sayfa yönlendirme
        } catch{

            showError('Çıkış yaparken bir sorun oluştu.');
        } finally {
            setLoading(false);
        }
    };
